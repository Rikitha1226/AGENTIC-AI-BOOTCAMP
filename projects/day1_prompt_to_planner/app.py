...
if not API_KEY:
    st.error("🔑 GOOGLE_API_KEY is missing. Please create a .env file with GOOGLE_API_KEY=your_key_here")
    st.stop()
...
st.write(
    "Give me a goal, and I’ll break it into small, smart steps for you.\n"
...
# -----------------------------
goal = st.text_input(
    "🎯 Enter your goal (e.g., 'Prepare for AI exam', 'Start a YouTube channel')",
    ""
)
...
include_schedule = st.checkbox("🗓️ Include a simple timeline (e.g., Week 1, Week 2)?", value=True)
tone = st.selectbox(
    "Tone of the plan",
...
if st.button("✨ Generate Plan"):
    ...
    if not goal.strip():
        ...
        with st.spinner("Thinking and planning your steps... 🤔"):
            try:
                ...
                st.caption(
                    "⚠️ This plan was generated by an AI agent. "
...
with st.sidebar:
    ...
    st.subheader("🔐 Safety")
    st.write(
        "✅ Your API key is loaded from `.env` (not hard-coded).\n"
        "✅ Do NOT share your key or push `.env` to GitHub.\n"
        "✅ This app does not store your inputs."
    )

    st.markdown("---")
    st.subheader("🧠 Next Step")
    st.write(
        "On Day 2, we’ll give this agent **memory** and the ability to use **external tools** (like APIs)."
    )
